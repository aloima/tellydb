...
static inline bool get_resp_command_name(Client *client, string_t *name, char *buf, int32_t *at, int32_t *size) {
  uint32_t len = 0;
...
static inline bool get_resp_command_argument(Client *client, string_t *arg, char *buf, int32_t *at, int32_t *size) {
  char *c;
  TAKE_BYTES(c, 1, false);
  if (VERY_UNLIKELY(*c != *RDT_BSTRING)) THROW_RESP_ERROR(client->id);
...
bool parse_resp_command(Client *client, char *buf, int32_t *at, int32_t *size, commanddata_t *command) {
  ...
  if (!get_resp_command_name(client, &command->name, buf, at, size)) return false;
  ...
  if (command->args.count != 0) {
    ...
    for (uint32_t i = 0; i < command->args.count; ++i) {
      ...
      const bool parsed = get_resp_command_argument(client, &command->args.data[i], buf, at, size);
